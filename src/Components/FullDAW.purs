...
import Lib (compileErrorsToString, compileUrl, initializeWags, loader, sanitizePS)
...
fullDAWExample =
  """module All.I.Need.Challenge where
...
render { audioUIState } =
  HH.div_
    [ HH.h3_ [ HH.text "Here's what we made" ]
    , HH.small_ [ HH.text "Press ", HH.b_ [HH.text "play"], HH.text " below to kick it off. The music can be edited live in the browser!" ]
    , HH.pre_
        [ HH.code
...
handleAction :: forall output m. MonadAff m => SineQuaNon -> Action -> H.HalogenM State Action () output m Unit
handleAction
  { name
  , setAlert
  , modules
  ...
  Initialize -> do
    maybeCs <- H.liftAff do
      hush <$>
        ( try $ makeAff \cb -> do
            compile
              { code: sanitizePS fullDAWExample
              , loader
              , compileUrl
              , ourFaultErrorCallback: cb <<< Left
              , yourFaultErrorCallback: cb <<< Left <<< error <<< show <<< compileErrorsToString
              , successCallback: cb <<< Right
              }
            mempty
        )
    { emitter, listener } <- H.liftEffect $ HS.create
    started <- H.liftEffect $ Ref.new false
